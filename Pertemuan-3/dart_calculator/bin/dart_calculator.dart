...
void main() {
  int jmlsmt = 0; // Inisialisasi variabel
  List<int> jmlmk = List.filled(14, 0); // Menyimpan jumlah mata kuliah per semester
    ...
    if (input != null) {
      ...
      if (jmlsmt < 2 || jmlsmt > 14) {
        ...
        for (int i = 0; i < jmlsmt; i++) {
          ...
          if (input != null) {
            jmlmk[i] = int.parse(input); // Menyimpan jumlah mata kuliah per semester
            if (jmlmk[i] < 2) {
  ...
  for (int i = 0; i < jmlsmt; i++) {
    ...
    for (int j = 0; j < jmlmk[i]; j++) { // Loop berdasarkan jumlah mata kuliah per semester
